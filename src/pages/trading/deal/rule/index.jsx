...
const handleAdd = async (type, values) => {
  ...
  try {
    !type ? await addDealRule({ ...values }) : await addDealDateGroup({ ...values });
    hide();
...
const handleUpdate = async (type, values) => {
  const hide = message.loading('正在更新');
  try {
    !type ? await updateDealRule({ ...values }) : await updateDealDateGroup({ ...values });
    hide();
