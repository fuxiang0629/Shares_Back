...
@connect(({ global, tabs, loading }) => ({
  global,
  tabs,
  setSelectedKeys: loading.effects['global/setSelectedKeys'],
}))
    ...
    return (
      <div>
        <DraggableTabs
          ...
          type="editable-card"
          onEdit={this.onEdit}
          onTabClick={(ev) => this.handleTabChange(ev)}
        >
          {pages.map((pane) => {
            return (
              <TabPane
                ...
                forceRender={false}
              >
                <div key={keys[pane.key]}>{activeKey == pane.key && pane.content}</div>
              </TabPane>
            );
          })}
        </DraggableTabs>
